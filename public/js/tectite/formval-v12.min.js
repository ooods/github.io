...
function TectiteFormValidator($){var validator=this;var s_ajax_img_path='';var s_error_submit="Форма не завершена. Исправьте ошибки перед отправкой, пожалуйста.";var s_error_field_empty="Пожалуйста, заполните это поле.";var s_error_radio_reqd="Пожалуйста, выберите один из вариантов.";var s_error_select_reqd="Пожалуйста, выберите значение для этого поля.";var s_error_checkbox_reqd="Этот флажок необходимо отметить.";String.prototype.tectite_ltrim=function(){return(this.replace(/^\s*/,""))};String.prototype.tectite_rtrim=function(){return(this.replace(/\s*$/,""))};String.prototype.tectite_trim=function(){return(this.tectite_ltrim().tectite_rtrim())};function GetArgs(){var args=new Object();var query=location.search.substring(1);var pairs=query.split("&");for(var i=0;i<pairs.length;i++){var pos=pairs[i].indexOf('=');if(pos==-1)continue;var argname=pairs[i].substring(0,pos);var value=pairs[i].substring(pos+1);value=decodeURIComponent(value);args[argname]=value}
return args}
...
function AjaxSetup(m_form,s_img_url){var j_form=$(m_form);var mesgs={loading_img:'<img src="ajax-loading.gif" alt="" />',done_img:'<img src="ajax-done.gif" alt="" />',done_mesg:'Сообщение отправлено.',error_img:'<img src="ajax-error.gif" alt="" />',error_mesg:'Ошибка!'};function AdjustImgTag(s_url,s_img){return(s_img.replace(/src="/,'src="'+s_url))}
...
catch(exc){res={Result:'FAILED',ErrorMesg:'Сервер вернул ошибку: JSON parse failed'}}
